#> Description: Places Muffin's lobby.
...
# Place Muffin's lobby template.
place template ogvz:lobby/muff1n_gauntlet ~-18 ~-10 ~-18
...
# Tell the admins which player placed Muffin's Gauntlet lobby.
tellraw @a[tag=ogvz.admin] [ \
  "", \
  {text:"SETUP: ",bold:true,color:"green"}, \
  {"selector":"@s",color:"green"}, \
  {text:" placed Muffin's Gauntlet Lobby.",color:"green"} \
]
