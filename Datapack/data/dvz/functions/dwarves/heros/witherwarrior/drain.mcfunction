...
execute as @s[tag=wither,scores={DVZ.drain.cool=1..}] run title @s actionbar ["",{"text":" * [Drain] You have ","color":"dark_red"},{"score":{"name":"@s","objective":"DVZ.drain.cool"},"color":"dark_red"},{"text":" seconds remaining!","color":"dark_red"}]
execute if entity @s[tag=wither,level=..29] run title @s actionbar {"text":" * [Drain] You need at least 30 Mana!","color":"dark_red"}
...
execute as @s[tag=DRAINuse] run title @s actionbar {"text":" * [Drain] Poof!","color":"red"}
