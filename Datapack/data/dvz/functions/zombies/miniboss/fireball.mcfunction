...
execute as @s[tag=ghast,scores={DVZ.ghast.cool=1..}] run tellraw @s ["",{"text":" * [Fireball] You have ","color":"dark_red"},{"score":{"name":"@s","objective":"DVZ.ghast.cool"},"color":"dark_red"},{"text":" seconds remaining!","color":"dark_red"}]
execute as @s if entity @s[tag=ghast,scores={DVZ.ghast.cool=0}] run tellraw @s {"text":" * [Fireball] Poof!","color":"red"}
