...
execute as @s at @s if block ~ ~ ~ #dvz:gothrough if entity @p[tag=ocelot,scores={DVZ.steal.cool=0},distance=..10] unless entity @p[tag=dwarves,distance=..2] run function dvz:zombies/ocelot/stealloop