...
execute as @s[tag=ocelot,scores={DVZ.disarm.cool=1..}] run tellraw @s ["",{"text":" * [Disarm] You have ","color":"dark_red"},{"score":{"name":"@s","objective":"DVZ.disarm.cool"},"color":"dark_red"},{"text":" seconds remaining!","color":"dark_red"}]

execute as @s if entity @s[tag=ocelot,scores={DVZ.disarm.cool=0}] run tellraw @s {"text":" * [Disarm] Poof!","color":"red"}
